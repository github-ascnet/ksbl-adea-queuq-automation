...
function Invoke-RenameUserAccount {
    ...
    try {
        ...
        foreach ($row in $rows) {
            try {
                $result = & $Context.Services.UserProvisioning.RenameUser $Context $row
                ...
                if ($result -and $result.PSObject.Properties['RequiresRetry'] -and [bool]$result.RequiresRetry) {
                    ...
                    Write-LogWarn -Logger $Context.Logger -Message "Invoke-RenameUserAccount: transient migration state for '$($row.AdObjectName)'. Scheduling retry after $retryMinutes minutes."
                    return New-JobRetryResult `
                        -Message "GenericUser.RenameAccount: transient migration state for '$($row.AdObjectName)'. $($result.Message)" `
                ...
                if ($result -and $result.PSObject.Properties['Success'] -and (-not [bool]$result.Success)) {
                    ...
                    Write-LogWarn -Logger $Context.Logger -Message "Invoke-RenameUserAccount failed for '$($row.AdObjectName)': $($result.Message)"
                    continue
                }
                ...
                $successResults += [pscustomobject]@{
                    AdObjectName       = $row.AdObjectName
                    TargetAdObjectName = $row.TargetAdObjectName
                    Message            = if ($result -and $result.PSObject.Properties['Message']) { $result.Message } else { "Row processed successfully." }
                    Result             = $result
                }

                Write-LogInfo -Logger $Context.Logger -Message "Invoke-RenameUserAccount succeeded for '$($row.AdObjectName)' -> '$($row.TargetAdObjectName)'."
            }
            catch {
                $failedResults += [pscustomobject]@{
                    Row          = $row
                ...
                Write-LogError -Logger $Context.Logger -Message "Invoke-RenameUserAccount failed for '$($row.AdObjectName)'." -Exception $_.Exception
            }
        }
