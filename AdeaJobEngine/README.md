...
## UserHomeDirectory / DFS-Finalisierung für PersonMailbox.CreateNonStandard

Die Legacy-Funktionen aus `current-scripts/Process-PersonMailboxJobs.ps1` für HomeDrive-, DFS- und ACL-Verarbeitung wurden in die modulare Struktur überführt.

Migrierte Legacy-Bausteine:
...
Die finale Phase von `PersonMailbox.CreateNonStandard` ruft diese Logik über `UserHomeDirectoryService` auf. Damit werden HomeDirectory-/DFS- und Applikations-/Desktop-Berechtigungen nicht mehr direkt im LongRunning-Handler verarbeitet, sondern über Service- und Gateway-Funktionen gekapselt.

Die relevante Konfiguration liegt unter `HomeDirectory` in `appsettings.json`:
...
## Modernisierung DFS/HomeDirectory ohne DFSUtil

Die HomeDirectory-/DFS-Finalisierung für `PersonMailbox.CreateNonStandard` verwendet keine `dfsutil.exe`-Aufrufe mehr. Die alte Logikkette aus `Process-PersonMailboxJobs.ps1` wurde in `UserHomeDirectoryService.psm1` gekapselt und technisch über Gateways umgesetzt.

Der Ablauf ist nun:

1. Bestehendes HomeDirectory über `Find-ExistingUserHomeDirectoryPathSafe` suchen.
2. Falls ein altes Verzeichnis mit `.delete_manually_user_left_company` gefunden wird, wird es kontrolliert reaktiviert.
3. Falls kein bestehendes Verzeichnis existiert, wird ein Ziel über `Get-HomeDriveSafe` gewählt.
4. Das physische HomeDirectory wird über `FileSystemGateway` erstellt.
5. HomeDirectory-ACLs sowie Application-/Desktop-ACLs werden über `FileSystemGateway` gesetzt.
6. Der DFS-Namespace-Link wird über DFSN-Cmdlets gekapselt (`Get-DfsnRoot`, `Get-DfsnFolderTarget`, `New-DfsnFolderTarget`, `Remove-DfsnFolderTarget`).

Die zentrale Orchestrierung erfolgt über:

- `Set-UserHomeDirectoryAndDfsTarget` in `shared/UserHomeDirectoryService.psm1`
- `Update-DfsShareSettingsSafe` in `infrastructure/DfsGateway.psm1`
